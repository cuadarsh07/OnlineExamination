...
import jk.proj.com.util.JDBCUtil;
...
public class StudentServicveImpl implements StudentService {
	...
	public String STUDENT_SEARCH_QUERY = "select * from StudentInfo_tbl";
	...
	public void addStudentInfo(Student stu) {
		...
		try {
			...
			ps1.setDate(6, new Date(stu.getDOB().getTime()));//
	...
	public List<Student> searchAllStudent() {
		...
		try {
			...
			while(rs1.next()) {
				...
				ps4.setInt(1, stu.getStudentId());
				ResultSet rs2 = ps4.executeQuery();
				while(rs2.next()) {
					 addres=new Address();// 3 object will be created 
					 addres.setAddId(rs2.getInt(1));
					 addres.setLane(rs2.getString(3));
